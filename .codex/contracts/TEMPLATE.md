...
- `qa-review`: `.codex/gates/qa-review.md`（或其他 `.codex/gates/*.md` 路径）
- `qa-runtime`: `.codex/gates/qa-runtime.md`（或其他 `.codex/gates/*.md` 路径）

本节路径会被脚本实际读取，不只是文档；请保持在 `.codex/gates/` 下，避免 `worktree_fingerprint` 自指失效。

## Gate Artifact Headers

- `status`: `pass | fail | blocked | not-required`
- `checked_at`: `YYYY-MM-DD`
- `contract_sha`: 当前 `.codex/contracts/active.md` 的 sha256
