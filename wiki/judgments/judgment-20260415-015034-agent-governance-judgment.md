...
confidence: "medium"
counter_evidence:
  - "Manual operator review can be sufficient while the runtime remains small and changes are reversible."
  - "Heavy governance loops can become ceremony if the system never performs autonomous state mutation."
invalidation_rule: "If future corpus evidence shows reliable multi-agent execution without review queues, execution receipts, or rollback paths, weaken this judgment."
next_signals:
...
## Judgment
- Multi-agent systems need explicit governance loops once they can mutate shared state, coordinate multiple actors, or file conclusions back into a knowledge base.

## Supporting Evidence
- `building-effective-agents` emphasizes evaluation and bounded workflows over unconstrained autonomy.
- `react-paper-abstract` highlights iterative tool reasoning, which increases the chance of stale evidence and action drift.
- `autogen-multi-agent-debate-pattern`, `crewai-agents-concept`, and the local runtime all point toward review, rollback, and audit surfaces when many agents contribute.

## Counter Evidence
- Governance layers can overload small teams if they are always-on regardless of risk.
- Some read-only agent workflows only need provenance, not a full repair loop.

## Open Questions
- Which controls should be mandatory for every action, and which should activate only for stateful repairs?
- How much governance can stay deterministic before semantic review becomes necessary?

## Confidence And Next Experiment
- Confidence is high because the corpus repeatedly connects execution power with review, replay, and rollback needs.
- Next experiment: measure whether review queue, aging report, and receipt-backed safe apply reduce operator recovery time on repair workflows.

## Counter Evidence
- Governance could be overfit if the runtime remains mostly deterministic and human-triggered.
- A future corpus with strong autonomous benchmarks might show lighter-weight controls are enough.
