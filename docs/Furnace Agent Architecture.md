...
Product Shell 投影见 `docs/Furnace Product Shell.md`。
...
- **Backend 选择**：`AIWIKI_LLM_BACKEND` 可省略，默认 `deepseek-api`；其他 backend 必须显式设置；无隐式 cross-backend fallback，也无同 backend model fallback 链。要换模型就显式设 `AIWIKI_LLM_MODEL`。
...
- 最小链：`advanced alchemy start` → `advanced alchemy distill` → `advanced alchemy finalize` → `advanced alchemy promote`（+ `revert` / `demote`）
