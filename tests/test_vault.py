...
class VaultBootstrapTests(unittest.TestCase):
    ...
    def test_bootstrap_new_vault_creates_launcher_plugin_and_shell_summary(self) -> None:
        ...
        plugin_styles = (self.target / ".obsidian" / "plugins" / "furnace-product-shell" / "styles.css").read_text(encoding="utf-8")
        ...
        self.assertTrue((self.target / "wiki" / "indexes" / "README.md").exists())
        self.assertIn("不是 SoT", (self.target / "wiki" / "indexes" / "README.md").read_text(encoding="utf-8"))
        ...
        self.assertIn("docs/", app_config["userIgnoreFilters"])
        ...
        self.assertIn('content: "产出";', snippet)
        self.assertIn('content: "报告";', snippet)
        self.assertNotIn('content: "全部报告";', snippet)
        self.assertNotIn('flatten output/reports', snippet)
        self.assertIn('.nav-folder[data-path="output/reports"] > .nav-folder-title > .nav-folder-title-content', snippet)
        self.assertIn('.tree-item[data-path="output/reports"] > .tree-item-self > .tree-item-inner', snippet)
        self.assertNotIn('/* hide output/reports from the daily file tree */', snippet)
        self.assertNotIn('.nav-folder-title[data-path="output/reports"] {\n  display: none !important;', snippet)
        self.assertIn('.aiwiki-report-leaf .inline-title', plugin_styles)
        self.assertIn('.workspace-leaf-content[data-type="markdown"][data-path^="output/reports/"] .inline-title', plugin_styles)
