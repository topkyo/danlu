...
contract_sha: <sha256 of .codex/contracts/active.md>
...
- `qa-review` 是否必需，由 `.codex/contracts/active.md` 中的 `Gate Requirements` 决定
- `qa-runtime` 是否必需，由 `.codex/contracts/active.md` 中的 `Gate Requirements` 决定
- `qa-review` / `qa-runtime` 的 artifact 路径由 contract 中的 `Gate Artifacts` 声明决定；脚本会实际读取这些路径
- artifact 路径必须保持在 `.codex/gates/*.md` 下，否则 `deploy-gate` 会拒绝放行
- 若 `qa-runtime` 在本轮不必需，建议仍写 artifact，并标记 `status: not-required`
- `contract_sha` 必须与当前 `.codex/contracts/active.md` 匹配，否则视为旧 artifact，不得放行
...
- 计算 `worktree_fingerprint` 时应排除 gate artifact 自身（`.codex/gates/*.md`），避免自指失效
