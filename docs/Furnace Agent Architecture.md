...
| 金丹最小链路 | partial | 当前 CLI 为 `alchemy-start / alchemy-distill / alchemy-finalize / alchemy-promote`，已落 `output/_candidates/elixirs/` 候选平面、`wiki/elixirs/` 持久平面、provenance、DAG 校验与 promote/revert/demote receipt；legacy migration 已有 read-only preview 与显式 apply baseline，superseded cleanup 已有 read-only preview，deletion apply 仍 deferred。 |
