...
- **ask**：你向炉子提问，生成一篇自由 Markdown 报告（`output/reports/*.md`）；默认 `deepseek-v4-flash` 下可经 DeepSeek Responses `web_search` 联网调研；LLM 按问题组织内容，不再强制六段骨架或多 format 分叉。
...
### 主入口：Obsidian Product Shell

打开 vault 后，左侧边栏的 **Furnace Product Shell** 是日常工作台（**Desktop Obsidian only**；iPad/iOS 不提供全功能移植）。首屏只暴露最常用的动作：
...
日常投料、提问、看报告都在 Product Shell 完成。

### 终端与自动化：`aiwiki` CLI

终端、脚本和自动化（watcher / nightly 服务）使用 `aiwiki` console script。在 vault 根目录下执行（`--root` 默认取当前目录，也可用 `AIWIKI_VAULT` 或显式 `--root` 指定）：

```bash
# 投料
aiwiki drop markdown --title "某券商会议纪要" --text "..."
aiwiki drop url https://example.com/article
aiwiki drop pdf /path/to/report.pdf
aiwiki drop image /path/to/chart.png
aiwiki drop repo https://github.com/org/repo.git
# 万能入口：一条 payload（URL / 本地路径 / 问题）→ 默认 LLM 计划再确定性执行
aiwiki drop https://github.com/org/repo
aiwiki drop plan https://github.com/org/repo   # 只看计划，不写 raw
...
- 财报 PDF → `aiwiki drop pdf 2025q2-report.pdf`
- 赛道文章 URL → `aiwiki drop url https://...`
- 会议纪要文字 → `aiwiki drop markdown --title "某专家访谈" --text "..."`
- 产业链图 → `aiwiki drop image supply-chain.png`
...
在默认 `deepseek-api` + `deepseek-v4-flash` 下，Ask 可触发 DeepSeek Responses 提供商侧 `web_search` 做联网调研（本地 vault 有料时也可补网）。网页证据只写入当次报告 frontmatter（`used_web_refs`）与 receipt，**不会**自动进入 `raw/`。若要长期保留某网页，请显式：

```bash
aiwiki drop url https://example.com/article
```

手选 `deepseek-v4-pro` 时 V1 无提供商联网，Ask 仅用 vault 内容；无有效证据时报告会标 `artifact_quality: no-evidence`，且不可 `file-back`。
...
产品面 `file-back` **只写 judgment**；CLI 没有 `--kind`。
...
aiwiki advanced review-page wiki/judgments/xxx.md --status confirmed --note "..."

# 废弃
aiwiki advanced review-page wiki/judgments/xxx.md --status discarded --note "..."
```
...
aiwiki advanced alchemy start --help
aiwiki advanced alchemy distill --help
aiwiki advanced alchemy finalize --help
```
...
处理：配置 key，然后跑 `aiwiki advanced llm-check --probe`。不影响确定性链路（投料、compile、today、lint）。
...
处理：先跑 `aiwiki advanced lint` 定位问题；再看 `.aiwiki/state/execution-receipts/` 找到失败命令；必要时手动修复原料或配置后重跑。
...
```bash
# 在 Obsidian 中直接打开 wiki/indexes/review-queue.md（由 compile 生成）
# 或用 CLI
aiwiki advanced review-queue
```
...
- P1-P5 稳定化清单
- `PYTHONPATH=src python3 -m aiwiki.cli ...` 这类开发命令（装 `aiwiki` console script 后不需要）
- `.aiwiki/state` 内部结构
