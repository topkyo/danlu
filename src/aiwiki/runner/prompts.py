...
from aiwiki.runner.clients import _client_backend_name, _client_model_name
...
def _ask_web_search_instruction(client: SupportsComplete | None) -> str:
    from aiwiki.llm import supports_web_search

    backend = _client_backend_name(client) if client is not None else ""
    model = _client_model_name(client) if client is not None else ""
    if supports_web_search(backend, model):
        return (
            "## Web Search\n"
            "Provider web_search is enabled for this run. Search when local vault evidence is "
            "insufficient or the query needs timely information.\n"
            "Every non-trivial claim must cite a vault path (`wiki/sources/...`, `wiki/judgments/...`, etc.) "
            "or a full `https://` URL.\n"
            "If you used web URLs, add a trailing `## 可选沉淀` section listing each URL with "
            "`aiwiki drop url <url>` (do not execute drops yourself).\n"
        )
    return (
        "## Web Search\n"
        "This model has no provider web_search. Use vault sources only.\n"
        "If vault evidence is insufficient, state the gap honestly and suggest switching to "
        "`deepseek-v4-flash` or using `aiwiki drop url` to ingest sources.\n"
        "Every non-trivial claim must cite a vault path; do not write facts without vault evidence.\n"
    )
...
def _build_ask_prompt(
    ...
    sections = [
        template,
        _ask_web_search_instruction(client),
        "## Target",
    ...
    if not source_pages:
        sections.append(
            "- No ranked source pages matched this query. Do not invent vault evidence.\n"
            "- If web_search is enabled, you may search for timely public information and cite full `https://` URLs.\n"
            "- If you still cannot answer, say the vault does not cover this topic and suggest "
            "`aiwiki drop url <url>` or `aiwiki drop markdown` before asking again."
        )
    else:
        omitted = 0
