...
from ..planner.state import load_query_route_telemetry
...
def _inbox_newer_than(root: Path, processed_at: datetime) -> bool:
    inbox = root / "raw" / "inbox"
    if not inbox.is_dir():
        return False
    processed_ts = processed_at.timestamp()
    for path in inbox.iterdir():
        if path.is_file() and path.stat().st_mtime > processed_ts + 5:
            return True
    return False


def _watcher_is_stale(root: Path, *, available: bool, processed_at: str) -> bool:
    """Stale means inbox has work the watcher has not processed.

    Idle-but-healthy watchers keep ``processed_at`` until the next digest
    change, so wall-clock age is not a stall signal.
    """
    if not available:
        return False
    parsed = parse_iso_datetime(processed_at)
    if parsed is None:
        return True
    return _inbox_newer_than(root, parsed)
...
def _build_watcher_summary(root: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "stale": _watcher_is_stale(root, available=available, processed_at=processed_at),
        "last_run_mode": mode,
        ...
        "note": (
            "Watcher only performs deterministic inbox processing; "
...
def thin_shell_summary_for_persist(summary: ShellSummary) -> ShellSummary:
    ...
    if isinstance(watcher, dict):
        thin_watcher = {
            ...
            "processed_at": watcher.get("processed_at") or "",
            "state_path": watcher.get("state_path") or "",
        }
