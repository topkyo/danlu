...
1. **知识 compiler**：`raw → wiki/sources → wiki/judgments|concepts → output` 的强分层；派生不可覆盖原始 source。产品回流默认 `advanced file-back` → `wiki/judgments/`；`wiki/derived/` 若存在多为 legacy 锚点，非现行写入目标。
2. **Receipt + revert + universal audit stream**：所有写入可审计、可回滚。
3. **Deterministic baseline**：不依赖 LLM 也能跑 `advanced compile / advanced lint / advanced run-nightly`。
