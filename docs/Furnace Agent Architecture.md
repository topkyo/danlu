...
炼丹炉的自主权现在是**分层可开关模型**：缺省 runtime policy 为 `autonomy_profile=strong`，维护、治理和 judgment review 默认开启；L3 核心写回与 heavy semantic apply 默认关闭。每层可通过 `.aiwiki/state/autonomy-policy.json` 或独立环境变量覆盖，所有自动采纳均写 receipt 支持 revert/audit。操作者可按层把对应 policy/env flag 改为 `false` / `0`，退回 dry-run preview 或更窄自动化半径。
...
| **L3 策略层** | `AUTO_ADOPT_L3` | 默认关闭；只自动登记 `metadata_only` proposal，核心 prompt/policy/schema 写回不无人值守发生 | 是 | 是 |
...
当前本机 full furnace / dogfood nightly profile（2026-05）默认开启每晚自动维护、治理、判断复核与受控学习；watcher 仍保持 deterministic-only，fallback、L3 核心写回和 heavy semantic apply 仍需显式启用，所有学习必须 receipt-gated、可审计、可回滚。
