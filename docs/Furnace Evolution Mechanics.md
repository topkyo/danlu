...
首版 planner-log 必须以 `mode=observe_only` 落地，且 `side_effects_allowed=false`；只有 signal replay、去重和 scope 计算连续通过 gate 后，才能开放 `dry_run`，最后才允许 `execute`。当前 `learning_threshold` 的 planner-log routing 只写 `generate-proposal` / `enqueue-heavy` 的 observe-only decision，不直接启动 proposal generation 或 lane apply。
