...
usage() {
  echo "Usage: $0 [--smoke-drop-note]" >&2
  echo "  --smoke-drop-note  drop a test note + compile (no maturity receipt)" >&2
}

while [[ $# -gt 0 ]]; do
  case "$1" in
    --smoke-drop-note) SMOKE_DROP=1; shift ;;
    -h|--help) usage; exit 0 ;;
...
if [[ "$SMOKE_DROP" == "1" ]]; then
  ...
  python3 -m aiwiki.cli --root "$VAULT" drop note --title "$NOTE_TITLE" --text "investing preflight smoke note"
