...
test("buildTodayFeed surfaces no-evidence as coverage-gap actions", () => {
  const summary = makeSummary({
    recent_outputs: [
      {
  ...
  const feed = buildTodayFeed(summary);
  expect(feed.filter((e) => e.kind === "report")).toHaveLength(0);
  const gaps = feed.filter((e) => e.kind === "action");
  expect(gaps).toHaveLength(1);
  expect(gaps[0].title).toBe("未覆盖：了解下今天的a股行情");
  expect(gaps[0].target).toBe("output/reports/a-share.md");
});

test("buildTodayFeed surfaces stale watcher as an action", () => {
  const summary = makeSummary({
  ...
  expect(actions[0].title).toBe("投喂监视已停滞");
});
