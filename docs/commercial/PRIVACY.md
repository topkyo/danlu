...
> 具体哪些内容被发送，取决于用户运行的命令（如 `advanced run-ask`）与所选上下文范围。炼丹炉不会 secretly 扩大上下文范围。
...
| 功能 | 网络访问目标 | 触发条件 |
|------|--------------|----------|
| LLM provider API | 用户配置的 provider endpoint | 运行 `advanced run-ask` 等显式 LLM 命令 |
| Ask 联网调研（`run-ask` + flash） | DeepSeek API（提供商侧 `web_search`；非用户任意 URL 直连） | `advanced run-ask` 且 backend=`deepseek-api`、model=`deepseek-v4-flash` |
...
- 2026-08-12：§4 增加 Ask / `run-ask` 在 flash+deepseek-api 下可能触发 DeepSeek 提供商侧 `web_search` 的说明。
