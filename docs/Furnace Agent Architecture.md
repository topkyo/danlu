...
当前 lane apply 白名单只包含已具备 deterministic receipt wrapper 的 `compile / lint / nightly`，且 runner 必须看到对应 dry-run step 明确 `apply_supported=true` 才能执行。`judge` 属于目标 phase，目前仅有独立 scoped dry-run preview，用于盘点 dirty scope 内的 judgment refresh candidates；它不会写 judgment/decision、receipt、audit 或调用 LLM。`judge / distill / review / propose` 在 receipt、audit 和 revert/不可回滚声明齐备前不得加入 lane apply 白名单。
