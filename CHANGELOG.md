...
### Removed
- 删除恒空字段 `planner_next_action` / `planner_next_action_id`（graph query / Ask 报告 / planner log / acceptance stdout）。
- 删除 `AIWIKI_MODEL_FALLBACK` / `--model-fallback` 与 `ModelFallbackClient`：不再做同 backend model retry 链；换模型必须显式设 `AIWIKI_LLM_MODEL`。
- 拆除 apply 执行管线（规格 `docs/specs/2026-08-13-oss-apply-pipeline-demolition.md`）：不再计算 repair-plan / patch-plan / rewrite apply-ready；review-queue 的 `machine_memory_actions` 仍只读观察动作。

### Fixed
- Ask receipt 的 `web_search_calls.query` 改读真实 API 的 `action.queries` 列表（仍兼容 `action.query` 字符串），并丢掉 `ws_call_id=` 伪 query。
- `file-back` 拒绝 `delivery_mode: llm-failed|deterministic-fallback`、失败 `llm_status` 与 `contains_llm_placeholder`，不只靠 `artifact_quality`；失败 stamp 补 `artifact_quality: degraded`。
- Ask 报告 `web_search_used` 写 YAML `true`/`false`，不再写成字符串 `"True"`。
- DeepSeek Responses：`used_web_refs` 兼收 `url_citation`；成功路径落原始 JSON；规范化时去掉 `#ws_call_id=` 等 fragment，避免 open_page URL 对不上正文。W2 follow-up Ask 的 `prompt_hash` 因 turn-1 报告 frontmatter 布尔序列化变化而重算。
- 老 vault LLM receipt 的 `rerun_command` 不再为已删事件（如 `run-compile-concept`）拼 CLI。
- `wiki/indexes/README.md` / bootstrap 模板不再声称 reconcile 会写 `wiki/rewrite-proposals/`；该目录不是预建产品层。
- Ask 证据口径：`used_refs` 判定与正文抽取对齐，`raw/` / `schema/` / `output/reports/` 也算 vault 证据；只引 raw 的合法报告不再被盖 `no-evidence`。
- Shell `constants.js` 对 `llm_settings.js` 的 concat 顺序加 Jest 门禁；`src/README.md` 补上 `llm_settings` 必须先于 `constants`。
- `advanced llm-check` 未配置时的 human 文案改用 status `message`（通常是缺 API key），不再误导去设 `AIWIKI_LLM_BACKEND`。
- apply 拆除碎屑：shell-summary 不再嵌冻结 planner 队列；compile 写回清空 live queue 并刷新时间戳；带 `-dry-run.json` 伴生文件的历史 execution bundle 可被 compile 清掉。
- OSS 公共契约求真（规格 `docs/specs/2026-08-13-oss-public-truth.md`）：删除零调用的 `AIWIKI_L3_AUTO_ADOPT_MIN_EVIDENCE`；`AIWIKI_DISABLE_AUTOMATION` 只停 LLM、不停 watcher；Active 文档不再声称 backend 必须显式设置、installer 会写 LLM backend、`file-back --kind`、或 `drop note` / `--bucket mm_actions` 幽灵名；炉心/repair/协议/Shell 单写者不再教已删除的 apply 路。
- KISS 复盘残留（计划 `docs/plans/2026-08-13-kiss-residue-watcher-repair.md`）：launchd watcher/nightly 改走 `/bin/bash`，避开 Homebrew bash + libhistory 签名失败导致的空转；watcher `stale` 只在 inbox 新于上次处理时亮起；零召回 Ask 仍走 LLM（保留 web_search），prompt 禁止编造 vault 证据并建议 `drop url` / `drop markdown`；repair 不再把已删除的 rewrite apply 当可执行队列。
- Ask 缺陷收口（计划 `docs/plans/2026-08-12-ask-defect-fixes.md`）：`used_refs` 改为正文真实引用；盖章只认真实引用；无关 confirmed judgment 不再从弱扩展来源注入；frontmatter 中文不再 `\uXXXX`；报告相对链接按存在性改写；失败/无证据报告复用同 query stem；watcher `stale` 进 Today；当日 `no-evidence` 以覆盖缺口卡出现。
- acceptance 环境隔离：`case_runner._ACCEPTANCE_HOST_LLM_ENV_KEYS` 扩充为剥离全部 provider key / backend / base-url env（deepseek / opencode / anthropic / openai）；此前只剥 opencode 时代变量，默认路由切到 `deepseek-api` 后，宿主 shell 导出的 `DEEPSEEK_API_KEY` 会让 universal drop planner 在测试里发真实 LLM 请求。
- `app_shell` watcher summary 的 `rerun_command` 指向已删除的 `auto-once` 死命令 → 移除该字段；LLM receipt rerun 命令前缀 `./scripts/aiwiki-launcher.sh` → `aiwiki`。
- R10 入口面残留：`shell_drift_warnings` 写入 thin shell-summary；删零调用 Shell API（`shell_search` / `shell_status_dashboard` / `shell_dashboard` / `shell_suggested_next_actions`）；Product Shell 不再读 `review_controls` / `execution_controls`，删 `rewriteControlItems` / `actionControlItems` 无入口链；autonomy 僵尸 flag 与 `memory/paths` corpus re-export 清理。计划：`docs/plans/2026-08-07-entry-surfaces-r10-cleanup.md`。
...
- alchemy promote receipt 补 `revert_supported: true`，audit 流不再误标不可 revert。
- `metrics_io._read_review_counts` 失败时打 warning（不再真静默吞错）；仍 best-effort 不崩 metrics。
- `_frontmatter_string_list` 四处拷贝统一为 `utils.markdown.frontmatter_string_list`。

### Added
- 双仓库：真源 `topkyo/danlu-private`；展示仓 `topkyo/danlu` 由 `scripts/export_public_tree.sh` 导出（不含 `docs/archive/`、`docs/plans/`、`PROGRESS.md`）。
- OSS 公开后卫生：`NOTICE`、`.gitattributes`、CI `ubuntu-latest` + `macos-latest`、README 产品/包/仓库/插件命名表。
- 开源发布切片：`CONTRIBUTING.md` / `SECURITY.md` / `.env.example` / issue 模板；`AGENTS.md` 去掉维护者家目录（改 `AGENTS.local.md.example`）；dogfood 脚本强制 `FURNACE_DOGFOOD_VAULT`；Product Shell `manifest.json` 不再提已删除的 launcher。
- Ask 联网调研（计划 `docs/plans/2026-08-12-ask-web-research.md`）：`deepseek-api` + `deepseek-v4-flash` 下 `run-ask` 可走 DeepSeek Responses `web_search`；报告 frontmatter 回写 `web_search_used` / `used_web_refs`；网页证据默认只进报告/receipt，显式 `drop url` 方可沉淀进 `raw/`。
- CLI 侧 `merge_plugin_data_env`：launchd / systemd / 终端等无交互运行时，从 vault 插件 `data.json` 读 LLM backend / model / key 补空环境位（process env 优先，不覆盖）。
- `scripts/pick_python.sh`：统一挑选 Python ≥3.10（wrapper 与 install 脚本共用；install 时把结果固化进服务环境 `AIWIKI_PYTHON`）。
- `aiwiki.corpus` 只读共享层（paths / scoring / ranks / parse / sections / snapshots / link_state）：`content ↛ memory` 且 `memory ↛ content`。
- Hub 单 seam：`render/ask_report.py`、`execution/file_back.py`、`content/output_artifacts.py`、`runner/workflows_ask_writeback.py`（`workflows_ask` 786→**334**；`ask_question` 巨函数仍记债）。
- `tests/test_cli_surfaces.py`：run-nightly / watch / review-queue / alchemy demote / drop pdf|image 六命令 library 烟测。
...
- `tests/test_llm_integration.py`：现行 **89** 条（含 ingest 幂等 / refresh、plan/execute、CJK、path containment、distill synthesizer、safe_fetch 重定向、Ask web_search / no-evidence）；历史起点为 42 条。
...
### Changed
- KISS follow-up（计划 `docs/plans/2026-08-13-kiss-followup-defaults-rewrite.md`）：产品默认 LLM 以 `config.py` 为 Python SoT、`llm_settings.js` 为 JS SoT，vault 模板与 `DEFAULT_SETTINGS` 引用常量；漂移测试钉死两端。compile 不再从 quality 累计无法 apply 的 concept_rewrite 提案，并 prune 本进程写出的旧提案页（弱概念仍走 lint）。
- KISS 瘦身（计划 `docs/plans/2026-08-12-kiss-complexity-slim.md`）：CLI 只留 `advanced alchemy <leaf>`，删除 `alchemy-*` 平铺别名与 argparse 私有 help hook；`LAYOUT_DIRS` 从 29 个预建目录收到 8 个必需空目录，其余按需 mkdir；Active 文档去掉已删除的 `AIWIKI_NIGHTLY_AUTO_ADOPT_*` no-op 叙事（`AIWIKI_L3_AUTO_ADOPT_MIN_EVIDENCE` 当时误标仍生效，已删）。
- Ask 联网调研 + 默认 flash（计划 `docs/plans/2026-08-12-ask-web-research.md`）：产品默认模型 `deepseek-v4-pro` → `deepseek-v4-flash`（`config.py` / Shell / vault 模板）；`deepseek-v4-pro` 仅 Shell/设置手动选（V1 无提供商 `web_search`）。无证据 Ask 报告标 `artifact_quality: no-evidence`；Today feed 不把 `no-evidence` 当成功产出；`file-back` 拒绝显式 `no-evidence` / `degraded` / `placeholder`（缺字段的旧报告仍可回流）。`used_web_refs` 只采 `web_search_call` 中的 URL，不采正文里的裸链接。PRIVACY / USER_GUIDE / Runtime Ops 同步。
- 单入口收口（计划 `docs/plans/2026-08-12-single-entry-deepseek-default.md`）：日常入口只承诺 Obsidian Product Shell；终端 / 自动化统一走 `aiwiki` console script。默认 LLM 路由 `opencode-api` → `deepseek-api`（DeepSeek 官方 API 直连；随后默认模型收为 `deepseek-v4-flash`，`deepseek-v4-pro` 仅手选；`opencode-api` 留作显式 escape hatch）。Product Shell 插件直接 spawn `<python> -m aiwiki.cli`（新 `bridge/python` 解析 ≥3.10 + GUI PATH 修补），插件设置 `launcherPath` → `runtimeRoot`（旧 key 自动迁移）。repo README 双语重写；vault README/HOME 模板去掉 launcher 命令块与 backend 路由叙事；`docs_consistency_check.sh` 加 launcher / 模板叙事防漂移 guard。
...
- verify 现行口径：acceptance **25** + llm-integration **88** + unit **172** + Jest **209**（coverage informational **71%**；历史 16/17/18/24、42/65/76/78/79、69%、72/81/143、153、154、160、164、166、168、170、171、174/177/186/189/201/204/206/207、208/210/212/213、215、87、89 等为沿革快照）。
...
- Rescan follow-up：见 Fixed；verify llm-integration 沿革曾为 **79**，现行 **87**。
...
### Removed
- `scripts/aiwiki-launcher.sh` 物理删除：vault bootstrap 不再生成 launcher（新 vault `data.json` 预置 `runtimeRoot`）；`sync-product-shell` 自动删除存量 vault 的旧 launcher 并回收空 `scripts/` 目录；`run_watch` / `run_nightly` / `run_launchd_*` / `install_*` 全部直连 CLI。
...
- **[Round 7 cross-review]**: `tests/fixtures/{planner_log, signals, signals_collector}/` 三个孤立目录共 42 文件（307 LOC）：acceptance/case_runner.py 与 `tests/test_acceptance_loop.py` 全部 case 均不引用此 fixtures，pure 孤立 cleanup，零风险（**17** case 为 historical 口径；Removed 段 **24** acceptance 亦为 historical；现行见 Unreleased：**25**）。
...
- `verify.sh all`（release 用）不再跑 pytest unit / coverage gate；现行为 7 步（scripts + product-shell-static + cli-smoke + smoke + python-static + acceptance + llm-integration，见 Unreleased 现行口径 **25** / **84** / **177** / **204**；本行 **24** / **85** / **81** / **203** 为 2026-07-15 historical 快照）。
