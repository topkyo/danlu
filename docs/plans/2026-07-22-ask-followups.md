...
- [ ] 确认 vault 无 `background_status ∈ {submitted,running}`（已预检：reports 无；仅 `分析下内容-2` 为 `completed`）
- [ ] 用 launcher 对 vault 跑：短 ask → 第二 ask 应被 Shell 单飞挡住（若仅 CLI 则串行两次记耗时）→ 中途 `drop` 投料仍成功
- [ ] 记录每次 wall time、exit、产物路径、是否 degraded
- [ ] 勿打印 API key；无凭据则 BLOCKED 并写清缺什么
...
- [ ] 删除对 `background_status ∈ {submitted,running}` 的隐藏/跳过逻辑（Python + Shell today_feed）
- [ ] 评估：`background-pending` / `llm_status=pending` 骨架是否仍需过滤——若仅历史 submit 产物，一并收紧或删除；保留 `degraded`/`llm-failed` 展示逻辑
- [ ] `workflows_ask_context.py` 若仅 scrub prompt 字段可保留
- [ ] 去掉 pending schema 的 `jobId`（serialize/hydrate/create/reset/tests）
- [ ] vault：可从 `分析下内容-2.md` frontmatter 去掉已无用的 `background_*`（可选，dogfood）
- [ ] **Verify:** `bash scripts/verify.sh product-shell-static python-static acceptance`
...
- [ ] 记一笔：`run-ask-submit/resume` + `background.py` 已退役（Ask sync-chat）
- [ ] 刷新过期计数（acceptance/Jest）若本文件快照仍写旧值
- [ ] 明确下一步仍是 WS 商业化/打磨，不扩后台任务
- [ ] **Verify:** `bash scripts/docs_consistency_check.sh`（若触及）或人工 diff 自检
