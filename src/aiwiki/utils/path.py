...
def next_available_stem(
    directory: Path,
    seed: str,
    suffix: str = ".md",
    reuse_existing: Callable[[Path], bool] | None = None,
) -> str:
    existing = directory / f"{seed}{suffix}"
    if existing.exists() and reuse_existing is not None and reuse_existing(existing):
        return seed
    candidate = seed
    index = 2
