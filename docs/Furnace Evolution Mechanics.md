...
- 既有命令的语义**不变**。
- 当前 lane apply 只允许 `compile / lint / nightly / review` 中已在对应 dry-run plan 出现且 `apply_supported=true` 的 primitives；`review` 仅限 heavy lane 显式 apply，默认不进入 `alchemy auto`，但可通过 `--lane heavy --primitive review` 显式 opt-in；`propose` 虽已有 direct scoped apply，但仍是 heavy/light lane 的 deferred apply primitive；`judge / distill` 仍为 deferred apply primitives。L3 prompt proposal candidate generation 已由独立 `l3-proposal-generate` baseline 与 direct `alchemy propose --apply` baseline 承接，保持 proposal-only 与人工 accept 边界。
...
| `aiwiki alchemy auto --dry-run\|--apply` | 当前：显式消费 `mode=execute` planner decisions，默认只调度已有 apply-supported deterministic lane primitives；`--lane heavy --primitive review` 可 opt-in 调度 review；不消费 observe-only decisions | lane primitive receipts + runtime history |
