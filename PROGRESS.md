...
- **实现**：
  - `scripts/install_user_service.sh:60,64-68,74,78-82,116-118`：5 个 auto-adopt env (`AUTO_APPLY_LIGHT` / `AUTO_ADOPT_L1/L2/L3/JUDGMENTS`) + `AIWIKI_NIGHTLY_FALLBACK_ENABLED` 默认 `1` → `0`；安装末尾 echo 提示 `AUTO_ADOPT_L1=1 ./scripts/install_user_service.sh` 显式启用模式。
  - `scripts/run_watch.sh:4-13` + `scripts/run_nightly.sh:4-13`：未设 `AIWIKI_VAULT` → stderr 错误 + `exit 1`，移除 fallback 到 repo root。
  - `scripts/run_nightly.sh:21`：`AIWIKI_NIGHTLY_FALLBACK_ENABLED` 默认 `0`。
  - `src/aiwiki/app_utils.py:43-47,140-150,160-176`：模块常量 `_LOCK_TIMEOUT_DEFAULT_SEC = 300` + `class LockTimeoutError(RuntimeError)` + `_resolve_lock_timeout()` env parse + clamp `[1,3600]` invalid 退默认；`runtime_write_lock` flock acquisition 改 `LOCK_NB` retry loop with `time.monotonic()` deadline + 0.1s sleep + 超时抛 `LockTimeoutError(message含 root + timeout + 提示)` + `handle.close()` on raise 无 FD 泄漏；reentrant depth-counter 路径在 retry loop 之前 short-circuit 返回，不受 timeout 影响。
  - `src/aiwiki/drop.py:420-423,1288-1310`：remote URL detection 覆盖 `http://` / `https://` / `git@` / `git://`，env `AIWIKI_ALLOW_REMOTE_REPO_DROP != "1"` → `ValueError("remote repo drop disabled; ...")`；`_clone_repo` `subprocess.run(... timeout=60)` + `TimeoutExpired` cleanup partial dir + clear error；本地 path 不受 env 影响。
- **测试**：新建 `tests/test_deploy_defaults.py` 11 测试 + `load_tests` bridge：
  - shell guard 4：`bash -n` 语法 + grep 默认值 / `run_watch.sh` + `run_nightly.sh` 未设 vault stderr+exit 1 / `run_nightly.sh` fallback default 0
  - lock 4：patched `fcntl.flock` 永远 `BlockingIOError` 触发 `LockTimeoutError` 走生产 retry loop / **真实 subprocess holder** 持锁 4s 主进程 1s timeout 验证 OS-level flock contention（响应 qa CONCERN，`tests/test_deploy_defaults.py:90`）/ reentrant 同进程二次 acquire 不 timeout / `_resolve_lock_timeout` env parse + clamp 边界（garbage→默认 / 99999→3600 / 0→1）
  - remote drop 3：env 缺失 ValueError / env 设了进 `_clone_repo`（patch 不真跑）/ `TimeoutExpired` cleanup partial dir
  - 调整既有：`tests/test_app.py:6295-6402` service script 断言匹配新默认；`tests/test_drop.py:164-181` remote clone 测试显式 opt-in env。
- **验证**：`bash scripts/verify.sh` PASS — **1838 unittest**（+11）+ 13/13 acceptance + branch coverage 92% / ruff + compileall clean。
- **qa-review**：fresh oracle session round 1 CONCERNS（lock timeout test patched-fcntl-only，未验证真实 OS-level lock contention 与 contract 描述 "real second writer hold lock" 不一致）→ 补 real subprocess holder 测试覆盖真实 flock contention（`fixer` resume `ses_206f9708cffedlyfSS1XATIhNk`）→ 用户接受。
- **Stop Lines**：0 `general` default protocol / 0 interactive prompt / 0 audit/receipt 自动 prune / 0 本地 `aiwiki` CLI 行为改动（`--root` / cwd / `drop-url` / `drop-image` vision / cache TTL 全留默认）/ 0 `safe_fetch` allowlist 语义 / 0 LLM backend/model defaults / 0 notify defaults / 0 schema 改动 / 0 lock primitive 重入语义 / 0 `_LLM_MAX_BYTES` / `_NOTIFY_MAX_BYTES` / drop file size cap / 0 CLI signature。
- **Migration**：既有 dogfood systemd 用户重装后 auto-adopt 默认全 off，需手动 `AUTO_ADOPT_L1=1 ./scripts/install_user_service.sh` 启用；`AIWIKI_VAULT` 必须显式设；`AIWIKI_ALLOW_REMOTE_REPO_DROP=1` 显式启用 remote git clone 路径。
- **Residual Risks**：`git clone --timeout=60` 对超大公共 repo 可能不够，无 timeout knob（不在本轮 scope）/ `LOCK_NB` retry loop 0.1s sleep 微小 CPU overhead（可接受）/ 旧代码若 catch broad 异常 → `LockTimeoutError` 行为变化（catch broad 是反 pattern 不在 stop line）。
- **归档**：contract `.codex/contracts/archive/round-92-deploy-defaults.md`；gate `.codex/gates/qa-review.md` 覆盖为 R92-DEPLOY-DEFAULTS pass。

**R92 batch 全部 P1 闭环：R92 / R92.1 / R92.2 / R92.3 / R92.4 / R92.5 / R92.6 / R92.7 = 8 rounds done。**

### Round 92.6 — Lock Audit Wide — 完成

- **目的**：闭环 oracle batch review 中的 R92-LOCK-AUDIT-WIDE P1。R92/R92.1 已锁定 `runner/alchemy.py`；R72/R74-79 已覆盖 drop_* / nightly_health / auto_adopt_* / append_*_history。本轮把 audit 扩到全仓：识别并锁定剩余的 SoT/state-mutating 入口，避免并发 CLI / nightly cron 双触发破坏共享状态文件。Tight + Standard scope 共 7 entrypoints。
- **实现**：
  - 5 个 decorator 加 `@runtime_write_operation`（首参均为 `root: Path`，符合 lock primitive root 提取方式）：
    - `src/aiwiki/app_compile_ops.py:309 set_active_protocol`（multi-file writer：`protocol.json` + `wiki/indexes/protocols.md` + wiki log）
    - `src/aiwiki/metrics_history.py:33 append_snapshot`（写 `.aiwiki/state/metrics-history.jsonl`）
    - `src/aiwiki/autonomy_policy.py:119 set_flag`（写 `.aiwiki/state/autonomy-policy.json`）
    - `src/aiwiki/runner/automation.py:22 auto_process_once`（写 `automation.json` + `automation-runs.jsonl` + 触发 compile/lint pipeline）
    - `src/aiwiki/app_shell/meta.py:214 write_shell_summary`（写 `.aiwiki/state/shell-summary.json` + product shell HTML）
  - 2 个 `apply=True` 分支包 `with runtime_write_lock(root):`，preview/dry-run 不取锁：
    - `src/aiwiki/execution/audit_preview.py:177 backfill_universal_audit_stream`（仅 apply=True 写 `.aiwiki/state/audit.jsonl`）
    - `src/aiwiki/planner/rollback.py:82 apply_planner_log_rollback_marker`（仅 apply=True append rollback marker）
  - 重入安全验证：lock primitive `src/aiwiki/app_utils.py:135-184` 用 `str(root.resolve())` 为 key + depth counter；`auto_process_once(root)` → `compile_wiki(root)` / `lint_wiki(root)` / `run_compile(root)` / `run_lint(root)` 全同 root；`write_shell_summary(context.root)` from `compile_wiki(context.root)` / `shell_status(root)` / `ask_question(root)` 全同 root；nested decorator 复用 depth counter 不死锁。
  - 新建 `tests/test_lock_audit_wide.py` 9 测试 + `load_tests` bridge：`_lock_depth(root)` 用 `str(root.resolve())` 查 `_RUNTIME_LOCKS` 对齐 primitive；5 decorator probe 各 patch 入口内部 helper assert lock depth >= 1；2 conditional ×（apply=True held / apply=False not-held）共 4 测试。
- **验证**：`bash scripts/verify.sh` PASS — **1827 unittest**（+9）+ 13/13 acceptance + branch coverage 92% / ruff + compileall clean。
- **qa-review**：fresh oracle session 一轮 PASS（`ses_20711d54affeLPsXBHWzpDeL2f`），逐项确认：5 decorator 首参签名匹配 / 2 conditional lock 仅在 write block / 重入 key 全同 root / 测试 probe 真实 patch 内部 helper / load_tests 收集 9 / `git diff --check` clean。
- **Stop Lines**：0 lock primitive (`runtime_write_lock` / `runtime_write_operation` / `_RUNTIME_LOCKS`) 语义改动 / 0 `runner/alchemy.py`（R92/R92.1 已覆盖）/ 0 `drop_*` (R72) / 0 `auto_adopt_*` (R79) / 0 `nightly_health` (R72) / 0 `append_*_history` (R74-79) / 0 receipt / manifest schema / 0 CLI signature / 0 `app_protocol.py` scaffold writers (Stretch，留 R92-LOCK-SCAFFOLD-EXTEND) / 0 `content/io.py:40 sync_manifest_with_raw`（已被 `ingest_source` 透传覆盖）/ 0 `runner/receipts.py:_append_jsonl_log` helper-level（caller 多已持锁，需另议）/ 0 lock on read-only / preview path / 0 broad `except Exception`。
- **Residual Risks**：helper-level decorator 不是事务边界——并发 writer 写半行 JSON 已被 `atomic_write_text` / `atomic_append_jsonl` 单行原子保护，但跨多文件仍非原子（不在本轮 scope，留 R92.2 类 TX 模式）/ Stretch entrypoints（`app_protocol.py` scaffold writers / `runner/receipts.py:_append_jsonl_log` helper）未覆盖 / `app_shell/meta.py:214` 加 helper-level decorator 与未来某 caller reentrant 路径冲突需后续审计。
- **归档**：contract `.codex/contracts/archive/round-92-lock-audit-wide.md`；gate `.codex/gates/qa-review.md` 覆盖为 R92-LOCK-AUDIT-WIDE pass。
