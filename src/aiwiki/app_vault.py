    ...
    return (
        "\n".join(
            [
                ...
                "- 默认 LLM 链路是 `auto -> codex-cli -> copilot-cli -> claude-cli`；若命中 `codex-cli` 且未显式设 model，effective model 默认 `gpt-5.4`。",
                "- `github-models-api` 是显式可选备用路，会优先复用 `gh auth token`；只有这条路当前能直接返回 usage。",
                "",
                "## 备用 CLI / 脚本入口",
                "",
                "```bash",
    ...
    return (
        "\n".join(
            [
                ...
                "5. 默认 LLM 链路是 `auto -> codex-cli -> copilot-cli -> claude-cli`；若命中 `codex-cli` 且未显式设 model，effective model 默认 `gpt-5.4`。",
                "6. `github-models-api` 是显式可选备用路，会优先复用 `gh auth token`；只有这条路当前能直接返回 usage。",
