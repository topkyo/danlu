...
- **Single writer, many readers**：同一时刻只允许一个 writer 触碰 `wiki/` / `.aiwiki/state/` / `output/control/` / `.aiwiki/derived/packs/`。
...
> **Historical / removed（2026-07-18）**：`runner/auto_adopt.py` 已删除；产品 `nightly` 仅 deterministic compile+lint。本节 L0–L3 / Judgment 分层与 env flag **为历史 operator 契约**，`AIWIKI_NIGHTLY_AUTO_ADOPT_*` / `AIWIKI_NIGHTLY_AUTO_APPLY_*` 设为 `1` **不再**触发无人值守采纳；字段名与 `.aiwiki/state/autonomy-policy.json` 保留仅为旧 state / installer 兼容。现行写回走 `advanced` CLI（review / apply / alchemy 等）。

炼丹炉曾采用 **agentic 分层可开关模型**：缺省 runtime policy 为 `autonomy_profile=agentic`，系统核心不自改；L1–L3 / judgment 层曾可通过 policy/env 缩窄，自动采纳均写 receipt 支持 revert/audit。

历史 env 映射（**legacy no-op**）：L0（维护层）曾由 `AIWIKI_NIGHTLY_AUTO_APPLY_LIGHT=1` 控制；L1-L3/Judgment 曾分别由 `AIWIKI_NIGHTLY_AUTO_ADOPT_L1/L2/L3/JUDGMENTS=1` 控制。**现行 nightly 固定只跑 L0 等价 deterministic compile+lint**，不读取上述 adopt flags。

红线表（**historical — env 现为 legacy no-op**）：

| 层级 | env flag | 曾自动做的事 | 写 receipt | 可 revert |
|---|---|---|---|---|
...
| **L3 策略层** | `AUTO_ADOPT_L3` | 非核心/metadata-only 学习登记；核心 prompt/policy/schema 写回须 human accept + hash-gated apply | 是 | 是 |
...
**现行（W8+）**：本机 nightly timer / `run-nightly` 仅 deterministic compile+lint + health write；watcher 仍 deterministic-only。治理 / judgment / L3 写回须 operator 显式 CLI，不得假装 nightly 默认开启 auto-adopt。
