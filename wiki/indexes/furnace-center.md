...
- 当前协议 review packs / memos / SOP：`1` / `5` / `10`
- 最近输出：`7`
- 本地控制面板：`output/control/furnace-center.html`

## 今天先做什么
1. 先处理 `5` 个 lifecycle concept backlog。
2. 先清理 `1` 个 judgment review action。
3. 优先复查 `1` 个升级项。

## 即刻可执行

### Execution Proposals
- `singleton-concept-base` | risk `medium` | targets `wiki/concepts/base.md`
...
- `overloaded-concept-url` | risk `high` | targets `wiki/concepts/url.md`
...
- `singleton-concept-base` | patch step `2`
  - `wiki/concepts/base.md` | mode `update` | sections `Summary, Related Sources, Related Concepts`
  - `wiki/indexes/concept-quality.md` | mode `review` | sections `Rewrite Priority, Open Questions`
...
- `overloaded-concept-the` | patch step `3`
  ...
  - `wiki/indexes/concept-quality.md` | mode `review` | sections `Merge Candidates, Rewrite Priority`
  - `wiki/indexes/rewrite-proposals.md` | mode `review` | sections `Merge Candidates, Rewrite Priority`
- `overloaded-concept-url` | patch step `3`
  - `wiki/concepts/url.md` | mode `rewrite` | sections `Summary, Related Sources, Related Concepts`
  - `wiki/indexes/concept-quality.md` | mode `review` | sections `Merge Candidates, Rewrite Priority`
  - `wiki/indexes/rewrite-proposals.md` | mode `review` | sections `Merge Candidates, Rewrite Priority`
...
### 当前缺口
- 有 `5` 个 protocol-related lifecycle concept backlog 尚未收敛。
...
- [Paper](../../wiki/concepts/paper.md) | kind `concept` | state `待回看` | invalidation `concept-evidence-gap` | active_corpora `7` | review_signals `active-quality-pressure` | reasons `invalidation-signal,concept-evidence-gap`
...
- `Review Recurring Agent Layers Judgment` | priority `high` | reasons `escalation-candidate, overdue-review` | command `PYTHONPATH=src python3 -m aiwiki.cli --root . review-page wiki/judgments/judgment-20260415-015034-recurring-agent-layers-judgment.md --status tracking`
...
- [SOP Draft · 扩展单节点概念 Base](../../output/packs/sop-drafts/singleton-concept-base.md) | kind `SOP Draft` | meta `medium`
