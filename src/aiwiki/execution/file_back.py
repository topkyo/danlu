...
_NON_DELIVERABLE_QUALITY = {"no-evidence", "degraded", "placeholder"}
_FAILED_DELIVERY_MODES = {"llm-failed", "deterministic-fallback"}
_FAILED_LLM_STATUSES = {
    "failed",
...
def _file_back_reject_reason(frontmatter: dict[str, Any]) -> str:
    quality = str(frontmatter.get("artifact_quality") or "").strip()
    if quality in _NON_DELIVERABLE_QUALITY:
        return f"artifact_quality={quality}"
    delivery = str(frontmatter.get("delivery_mode") or "").strip()
    if delivery in _FAILED_DELIVERY_MODES:
        return f"delivery_mode={delivery}"
    llm_status = str(frontmatter.get("llm_status") or "").strip()
    if llm_status in _FAILED_LLM_STATUSES:
        return f"llm_status={llm_status}"
    placeholder = frontmatter.get("contains_llm_placeholder")
    if placeholder is True or str(placeholder or "").strip().lower() in {"1", "true", "yes"}:
        return "contains_llm_placeholder"
    return ""
...
def file_back(
    ...
    reject_reason = _file_back_reject_reason(original_frontmatter)
    if reject_reason:
        raise ValueError(
            "Cannot file back a no-evidence or non-deliverable report into wiki/judgments "
            f"({reject_reason}). "
            "无证据或失败报告不能沉淀为 judgment；请先投喂来源、重新 run-ask，或仅保留在 output/reports/。"
        )
