...
def _register_advanced_parsers(subparsers: argparse._SubParsersAction) -> None:
    ...
    review_queue_parser.add_argument(
        "--bucket",
        help="Filter to one sub-bucket name (e.g. concept_backlog, revisit, machine_memory_actions, counter_evidence, drift).",
    )
    review_queue_parser.add_argument(
        "--limit",
        type=int,
...
def _register_drop_subcommand_parsers(subparsers: argparse._SubParsersAction) -> None:
    ...
    _configure_drop_markdown_parser(drop_markdown_parser)
    drop_markdown_parser.set_defaults(handler_command="drop-markdown")
...
def _configure_drop_markdown_parser(parser: argparse.ArgumentParser) -> None:
    parser.add_argument(
