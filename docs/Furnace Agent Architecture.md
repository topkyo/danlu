...
| universal audit stream | partial | 当前已有 `aiwiki audit-preview --dry-run` 只读归一化 execution receipts、LLM receipts、runtime history 与 protocol-learning aging audit；目标 `.aiwiki/state/audit.jsonl` append-only 写入仍 planned。 |
