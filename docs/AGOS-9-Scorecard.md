...
| LLM integration | **88** passed | `bash scripts/verify.sh llm-integration` |
| Unit（library 级） | **172** passed | `bash scripts/verify.sh unit` |
| Product Shell Jest | **209** passed | `bash scripts/verify.sh product-shell-static` |
...
| Product Shell | 12% | 9.3 | yes | Jest **209** + bundle drift 硬门禁 + sync `run-ask` 单飞 + Today-first |
...
| LLM reliability | 12% | 9.0 | no | llm-integration **88** + receipt 聚合 |
...
| 3 | LLM integration | **88** — `bash scripts/verify.sh llm-integration` |
| 4 | Product Shell | Jest **209** + bundle drift gate — `bash scripts/verify.sh product-shell-static` |
| 5 | Unit | **172** — `bash scripts/verify.sh unit` |
...
历史 / thesis：`docs/Furnace Elixir.md`
