...
def prune_compile_generated_rewrite_proposal_pages(root: Path) -> None:
...
def compile_runtime_phase(context: CompileContext) -> None:
    ...
    planner_state = load_planner_state(context.root)
    planner_state["state_path"] = relative_path(context.root, planner_state_path(context.root))
    planner_state["generated_at"] = str(context.compiled_at)
    context.write_maintenance_artifact(planner_state_path(context.root), planner_state)
    prune_compile_generated_rewrite_proposal_pages(context.root)
    remove_stale_generated_execution_proposal_pages(context.root, set())
    remove_stale_generated_execution_bundle_files(context.root, set())
