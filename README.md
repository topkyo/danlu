...
炼丹炉是一套面向投资研究、技术研发和高价值决策场景的 `local-first` 知识复利操作系统。它把网页、PDF、图片、repo 和本地笔记等原料，持续编译成可追溯的 `wiki`、`machine memory`、`decision / judgment` 和可回流输出；它不是静态笔记库，也不是一次性 RAG 问答器，而是一个帮助你长期积累 thesis、研究结论、技术判断和复审历史的认知系统。
...
- 上限路线图：[Furnace Ceiling Roadmap.md](<./wiki/indexes/Furnace Ceiling Roadmap.md>)
