...
def test_from_env_ignores_stale_model_fallback_env(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("AIWIKI_LLM_BACKEND", "opencode-api")
    monkeypatch.setenv("AIWIKI_OPENCODE_API_KEY", "k")
    monkeypatch.setenv("AIWIKI_LLM_MODEL", "primary-model")
    monkeypatch.setenv("AIWIKI_MODEL_FALLBACK", "secondary-model,tertiary-model")

    config = LLMConfig.from_env()
    status = LLMConfig.status_from_env()

    assert config.model == "primary-model"
    assert "model_fallback_chain" not in config.redacted()
    assert "model_retry_chain" not in config.redacted()
    assert "model_fallback_chain" not in status
    assert "model_retry_chain" not in status
...
# ---------------------------------------------------------------------------
# 6. _retry_ask_prompt_profile state machine decision
# ---------------------------------------------------------------------------
...
def test_mark_run_ask_artifact_degraded_writes_failure_notice(tmp_path: Path) -> None:
    ...
    assert frontmatter.get("artifact_quality") == "degraded"
...
def _valid_report_markdown() -> str:
    return (
        "---\ntitle: stub report\n---\n\n"
        "# Stub Answer\n\n"
        "Deterministic stub body for source-a (`wiki/sources/source-a.md`).\n"
    )
...
def test_complete_run_ask_artifact_web_search_refs_stamps_deliverable(tmp_path: Path) -> None:
    """Responses-style completion with used_web_refs → web_search frontmatter + deliverable stamp."""
    ...
    client = _StatefulMockClient(
        ...
        [
            CompletionResult(
                text=report_body,
                response_id="stub-web",
                usage={},
                web_search_used=True,
                used_web_refs=("https://finance.example.com/market",),
            ),
        ],
    )
    ...
    final_text = target.read_text(encoding="utf-8")
    frontmatter = parse_frontmatter(final_text)
    assert frontmatter.get("web_search_used") is True
    assert frontmatter.get("used_web_refs") == ["https://finance.example.com/market"]
    assert frontmatter.get("artifact_quality") == "deliverable"


def test_complete_run_ask_artifact_refusal_stamps_no_evidence_and_blocks_file_back(
    tmp_path: Path,
) -> None:
    """Refusal body with empty refs → no-evidence stamp; file-back must fail closed."""

    from aiwiki.execution.file_back import file_back

    vault = _copy_fixture_vault(tmp_path)
    artifact = ask_question(vault, "today market", "report")
    target = vault / artifact["path"]
    ...
    client = _StatefulMockClient(
        _mock_config(),
        [CompletionResult(text=refusal_body, response_id="stub-refusal", usage={})],
    )

    payload = _complete_run_ask_artifact(
        vault,
        artifact=artifact,
        question="today market",
        output_format="report",
        client=client,
    )
    ...
    final_text = target.read_text(encoding="utf-8")
    frontmatter = parse_frontmatter(final_text)
    assert frontmatter.get("artifact_quality") == "no-evidence"
    assert frontmatter.get("used_web_refs") == []
    assert frontmatter.get("web_search_used") in {None, "False", False, ""}

    artifact_ref = str(artifact["path"])
    with pytest.raises(ValueError, match="no-evidence|无证据"):
        file_back(vault, artifact_ref)
...
def test_build_llm_rerun_command_returns_empty_for_deleted_events() -> None:
    from aiwiki.app_shell.helpers import _build_llm_rerun_command

    cmd = _build_llm_rerun_command(
        {"event": "run-compile-concept", "target": "wiki/concepts/alpha.md"}
    )
    assert cmd == ""
...
def test_drop_note_is_rejected_not_a_subcommand() -> None:
    from aiwiki.cli.universal_input import _rewrite_universal_drop_argv

    with pytest.raises(SystemExit) as caught:
        _rewrite_universal_drop_argv(["drop", "note", "--text", "hi"])
    assert caught.value.code == 2


def test_drop_markdown_still_passes_through_as_typed_subcommand() -> None:
    from aiwiki.cli.universal_input import _rewrite_universal_drop_argv

    rewritten = _rewrite_universal_drop_argv(["drop", "markdown", "--text", "hi"])
    assert rewritten == ["drop", "markdown", "--text", "hi"]
...
def test_prune_rewrite_proposals_preserves_user_notes(tmp_path: Path) -> None:
    """Stale generated proposal pages are pruned; user notes in the same dir survive.

    Regression for the ownership guard now in ``prune_compile_generated_rewrite_proposal_pages``:
    ...
    from aiwiki.compile.runtime_step import prune_compile_generated_rewrite_proposal_pages
    from aiwiki.protocol.scaffold import ensure_layout
    ...
    prune_compile_generated_rewrite_proposal_pages(tmp_path)
...
def test_prune_rewrite_does_not_accumulate_unactionable_proposals(tmp_path: Path) -> None:
    from aiwiki.compile.runtime_step import prune_compile_generated_rewrite_proposal_pages
    from aiwiki.protocol.scaffold import ensure_layout
    ...
    proposal_dir = tmp_path / "wiki" / "rewrite-proposals"
    proposal_dir.mkdir(parents=True, exist_ok=True)
    generated = proposal_dir / "weak-concept.md"
    generated.write_text(
        "---\n"
        "id: rewrite-proposal-weak-concept\n"
        "kind: rewrite-proposal\n"
        "generated_by: aiwiki-run-compile\n"
        "---\n\n# Idle proposal\n",
        encoding="utf-8",
