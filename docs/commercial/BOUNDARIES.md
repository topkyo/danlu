...
- **Runtime CLI**：`aiwiki` 全部命令，包括 `drop`、`today`、`advanced` 及其下全部 operator 命令（含 `advanced metrics` / `advanced compile` / `advanced review-page` 等）。
...
- **治理链**：`advanced review-page` / `advanced file-back` / `advanced run-nightly` 与 aging / escalation / repair；L3 `apply` / `revert` 等产品 CLI 已删，审计走 library receipt。
- **金丹机制**：`advanced alchemy start|distill|finalize|promote|revert|demote` 及 elixir 生命周期管理。
- **Deterministic baseline**：不依赖 LLM 也能运行的 `advanced compile`、`advanced lint`、本地统计等路径。
