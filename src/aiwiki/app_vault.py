    ...
    return (
        "\n".join(
            [
                ...
                "- 默认 LLM 链路是 `auto -> codex-cli -> gpt-5.4`；CLI 路径下 usage/token 统计当前不可见。",
                "",
                "## 备用 CLI / 脚本入口",
                "",
                "```bash",
    ...
    return (
        "\n".join(
            [
                ...
                "5. 默认 LLM 链路是 `auto -> codex-cli -> gpt-5.4`；如果走 CLI，会显示 backend/model，但当前拿不到精确 token usage。",
                "6. 运行 `compile`，再从 `Ask / Review / Execution` 开始当天工作。",
                "7. 写操作遵守单写约束：不要同时在 Obsidian 和终端里各跑一个写命令。",
                "",
                "## 路径职责",
                "",
                "- `raw/inbox/`：输入材料",
