...
- 常用 target：`scripts`、`smoke`、`python-static`、`acceptance`、`cli-smoke`、`product-shell-static`、`all`
  ...
  - `all` 走 `scripts + product-shell-static + cli-smoke + smoke + python-static + acceptance`（≈ 20–25 s，**含 acceptance 24 fixture replay + Product Shell Jest 168**）；不含 pytest / coverage gate
...
- `tests/` 范围已收缩到 acceptance-only：`tests/test_acceptance_loop.py` + `tests/acceptance/` + `tests/fixtures/`，由 `bash scripts/verify.sh` 默认 `all` 跑 **24** acceptance tests（16 `case_*` fixture dirs + path safety 等）；旧 144 个 pytest 单元测试文件（118 顶层 + 26 `tests/unit/`，约 56k LOC）作为 contract 已 retire，`coverage>=7.6,<8` 已从 dev deps 中移除
