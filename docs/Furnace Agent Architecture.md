...
| heavy/light alchemy lane | partial | 已有 `aiwiki alchemy heavy|light <scope> --dry-run` 只读 preview，`--apply --action-id ...` 到既有 receipted low-risk action batch 的显式桥接，`--apply --primitive compile|lint|nightly` 的 deterministic receipt wrapper，以及 `aiwiki alchemy auto --dry-run|--apply` 对 execute-mode planner decisions 的显式调度入口；lane apply 会写 `alchemy-lane-started / alchemy-lane-completed` runtime-history audit events，scheduler apply 会写 `alchemy-auto-scheduler` runtime-history audit event，lane primitive receipt 已显式携带 planner trace 与 execution receipt history audit metadata；`review` 已有直接 scoped apply baseline，可显式写 review queue managed section 与 receipt/audit，但尚未加入 lane/auto apply；`judge/distill/propose` 目前仍只有独立 scoped dry-run preview 和 deferred apply contract metadata，不 apply、不写 judgment/decision、elixir candidate、proposal plane、receipt/audit 或调用 LLM。 |
...
当前 lane apply 白名单只包含已具备 deterministic receipt wrapper 的 `compile / lint / nightly`，且 runner 必须看到对应 dry-run step 明确 `apply_supported=true` 才能执行。`judge / distill / review / propose` 属于目标 phase，目前都有独立 scoped dry-run preview，用于盘点 dirty scope 内的 judgment refresh candidates、elixir refresh candidates、review enqueue candidates 与 proposal opportunities；preview 会暴露 `apply_contract` metadata（write surfaces、receipt schema、audit schema、revert policy、idempotency key、backend policy）。其中 `review` 已支持直接 `aiwiki alchemy review <scope> --apply`，只写 `wiki/indexes/review-queue.md` 的 managed section 与 receipt/audit/runtime history，不调用 LLM，也不加入 lane/auto apply；`judge / distill / propose` 仍保持 deferred，不写 judgment/decision、elixir candidate、proposal plane、receipt、audit 或调用 LLM。`judge / distill / propose` 在对应 contract 进入 executable 状态前不得加入 lane apply 白名单；`review` 在 lane/auto 集成单独完成前也不得加入 lane apply 白名单。
