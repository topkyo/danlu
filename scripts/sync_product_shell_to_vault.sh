...
#   - The default dogfood vault path can be overridden with the
#     FURNACE_DOGFOOD_VAULT environment variable for testing.
set -euo pipefail
...
VAULT_BASE="${FURNACE_DOGFOOD_VAULT:-/Users/ht/Library/Mobile Documents/iCloud~md~obsidian/Documents/炼丹炉/.obsidian/plugins/$PLUGIN_ID}"
...
if [[ ! -d "$VAULT_BASE" ]]; then
  ...
  echo "override with FURNACE_DOGFOOD_VAULT=<path-to-vault-plugins-dir>" >&2
  exit 1
fi
