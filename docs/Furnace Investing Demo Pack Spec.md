...
- **Mac / Desktop 是主产品**：完整 ingest、LLM、`advanced compile`、review（`advanced review-page` / `advanced file-back`）、金丹治理（`advanced alchemy start|distill|finalize|promote|revert|demote`）与 library receipt 语义，以及 desktop drain，都在本机 runtime 上执行。
