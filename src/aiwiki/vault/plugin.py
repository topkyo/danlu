...
def _build_product_shell_bundle(runtime_root: Path) -> None:
    ...
    try:
        completed = subprocess.run(
            ...
            text=True,
            check=False,
            timeout=120,  # build.sh subprocess cap, not LLM timeout
        )
...
def sync_product_shell_plugin(runtime_root: Path, target_root: Path) -> dict[str, Any]:
    ...
    # The vault-local launcher script was retired: the plugin now spawns the
    # runtime CLI directly. Remove the stale copy by exact managed path.
    removed_files: list[str] = []
    legacy_launcher = target_root / "scripts" / "aiwiki-launcher.sh"
    if legacy_launcher.is_file():
        legacy_launcher.unlink()
        removed_files.append("scripts/aiwiki-launcher.sh")
        try:
            legacy_launcher.parent.rmdir()  # only succeeds when scripts/ is empty
        except OSError:
            pass
