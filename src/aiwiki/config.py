...
WEB_SEARCH_SUPPORTED_MODELS = frozenset({"deepseek-v4-flash"})
...
def _read_env() -> dict[str, Any]:
    ...
    timeout_seconds = _int_env("AIWIKI_LLM_TIMEOUT", DEFAULT_LLM_TIMEOUT_SECONDS)
...
# Mirrors the Product Shell JS `llm_settings.js` provider profiles. The plugin
# injects these env vars when it spawns the CLI itself; this fallback covers
# terminal / launchd / systemd runs (previously done by the vault launcher).
_PLUGIN_LLM_PROFILES = (
    (
        BACKEND_DEEPSEEK_API,
...
PLUGIN_DATA_RELATIVE_PATH = Path(".obsidian/plugins/furnace-product-shell/data.json")


def merge_plugin_data_env(root: Path) -> None:
    """Fill empty LLM env slots from the vault's Product Shell data.json.

    Product Shell injects these vars when it spawns the CLI itself; this
    fallback keeps terminal / launchd / systemd runs working now that the
    vault launcher script is gone. Never overwrites non-empty env values.
    """
    path = Path(root) / PLUGIN_DATA_RELATIVE_PATH
    if not path.is_file():
        return
    try:
        ...
        _logger.warning(
            "Could not parse Product Shell data.json at %s (%s); skipping env merge",
            path,
            exc,
        )
        return
    ...
    if not isinstance(settings, dict):
        return
    profiles = {item[0]: item for item in _PLUGIN_LLM_PROFILES}
    # An explicit process-env backend wins outright: model / key / base-url are
    # then filled from THAT backend's profile so the pair stays consistent.
    env_backend = (os.environ.get("AIWIKI_LLM_BACKEND") or "").strip().lower()
    backend = env_backend if env_backend in profiles else ""
    if not backend:
        data_backend = str(settings.get("llmBackend") or "").strip().lower()
        if data_backend and data_backend not in profiles:
            _logger.warning(
                "Unknown llmBackend %r in Product Shell data.json at %s; falling back to %s",
                data_backend,
                path,
                DEFAULT_BACKEND,
            )
        backend = data_backend if data_backend in profiles else DEFAULT_BACKEND
    _, profile_model, key_setting, key_env, base_setting, base_env = profiles[backend]
    default_models = {item[1] for item in _PLUGIN_LLM_PROFILES if item[1]}
    default_models.add(DEFAULT_OPENCODE_MODEL)
    model = str(settings.get("llmModel") or "").strip()
    if profile_model and model and model != profile_model and model in default_models:
        if not (backend == BACKEND_DEEPSEEK_API and model == DEFAULT_OPENCODE_MODEL):
            model = profile_model
    exports = [("AIWIKI_LLM_BACKEND", backend), ("AIWIKI_LLM_MODEL", model or profile_model)]
    if key_setting and key_env:
        exports.append((key_env, str(settings.get(key_setting) or "")))
    if base_setting and base_env:
        exports.append((base_env, str(settings.get(base_setting) or "")))
    for name, raw_value in exports:
