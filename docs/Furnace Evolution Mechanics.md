...
当前实现状态：dry-run 可以预览上述目标序列，但 lane `--apply --primitive` 只支持已 receipt 化的 `compile / lint / nightly / review / propose` primitives，其中 `review` 与 `propose` 只允许 heavy lane 显式执行。Runner 执行前必须确认对应 dry-run step 存在且 `apply_supported=true`；`alchemy auto --dry-run|--apply` 只消费 `mode=execute` planner-log decisions，默认只调度已有 apply-supported deterministic primitives；若 operator 显式传入 `--lane heavy --primitive review`，auto 可 opt-in 调度 review，但 light lane 仍不允许 review，且 auto 仍不选择 propose。L3 proposal 生成通过独立的 `l3-proposal-generate --dry-run|--apply` 消费 execute-mode `generate-proposal` decisions，或通过 direct `aiwiki alchemy propose <scope> --apply` / explicit heavy lane `--primitive propose` 从 scoped dirty preview 生成 prompt proposal 候选；这些路径都不写目标 prompt/policy 文件。`aiwiki alchemy judge <scope> --dry-run` 当前只读盘点 heavy dirty scope 内的 judgment refresh candidates，`aiwiki alchemy distill <scope> --dry-run` 当前只读盘点 elixir refresh candidates，`aiwiki alchemy review <scope> --dry-run` 当前只读盘点 review enqueue candidates，`aiwiki alchemy review <scope> --apply`、`aiwiki alchemy heavy <scope> --apply --primitive review` 与显式 auto heavy review opt-in 可写入 review queue managed section 并生成 receipt/runtime-history/universal-audit，`aiwiki alchemy propose <scope> --dry-run|--apply` 与 `aiwiki alchemy heavy <scope> --apply --primitive propose` 当前可盘点 proposal opportunities，并在 apply 时只写 L3 proposal plane、proposal-generation receipt、runtime-history 和 universal audit，不调用 LLM、不写 prompt/policy 目标文件、不消费 `generate-proposal` decisions。`judge / distill` 仍返回 `apply_supported=false` / `missing_receipted_scoped_contract` 并携带 deferred `apply_contract` metadata。
...
- 既有命令的语义**不变**。
- 当前 lane apply 只允许 `compile / lint / nightly / review / propose` 中已在对应 dry-run plan 出现且 `apply_supported=true` 的 primitives；`review` 与 `propose` 仅限 heavy lane 显式 apply，默认不进入 `alchemy auto`；`review` 可通过 `--lane heavy --primitive review` 显式 auto opt-in，但 `propose` 仍不进入 auto；`judge / distill` 仍为 deferred apply primitives。L3 prompt proposal candidate generation 已由独立 `l3-proposal-generate` baseline、direct `alchemy propose --apply` baseline 与 explicit heavy lane `--primitive propose` 承接，保持 proposal-only 与人工 accept 边界。
...
| `aiwiki alchemy auto --dry-run\|--apply` | 当前：显式消费 `mode=execute` planner decisions，默认只调度已有 apply-supported deterministic lane primitives；`--lane heavy --primitive review` 可 opt-in 调度 review；`propose` 仍不进入 auto；不消费 observe-only decisions | lane primitive receipts + runtime history |
