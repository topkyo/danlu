...
| universal audit stream | partial | 当前已有 `aiwiki audit-preview --dry-run` 只读归一化 execution receipts、LLM receipts、runtime history 与 protocol-learning aging audit，并可通过 `aiwiki audit-backfill --apply` 显式 append 缺失 records 到 `.aiwiki/state/audit.jsonl`；execution receipt、runtime history 与 LLM receipt writer 已直接 append universal audit record，protocol-learning aging source writer 尚未直接写 universal audit stream。 |
