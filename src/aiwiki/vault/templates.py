    ...
    return (
        "\n".join(
            [
                ...
                "- 左侧文件树是用户视图：默认露出投料收件箱、报告、沉淀（`wiki/judgments`）、金丹（`wiki/elixirs`）；其余 runtime 分层仍隐藏。",
                ...
                './scripts/aiwiki-launcher.sh advanced run-ask "今天最重要的变化是什么？" --format report',
                ...
                "- `schema/`：运行时规则和协议",
                "- `.aiwiki/`：状态、缓存、日志",
                "- 普通用户文件树默认露出 `raw/inbox` 收件箱、`output/reports` 报告、`wiki/judgments` 沉淀、`wiki/elixirs` 金丹；其余运行时分层由 Product Shell、更多工具和 CLI 间接打开。",
    ...
    return (
        "\n".join(
            [
                ...
                "",
                "- [[README|使用说明]]",
                "- [[wiki/indexes/furnace-center|炉心面板（先跑 advanced compile 再打开）]]",
                "- [[wiki/indexes/README|索引策略（compile 后生成面板页）]]",
                "",
                "`wiki/indexes/` 下的炉心 / 审阅 / 判断资产等面板页由 `advanced compile` 生成；先跑 advanced compile 再打开。",
                "",
                "## 备用命令",
                "",
                "```bash",
