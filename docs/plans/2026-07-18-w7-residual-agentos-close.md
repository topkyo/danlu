...
- [ ] Nightly path must not call `workflows.run_compile()` / LLM `run_lint()` via debt_autopilot content digestion
- [ ] Prefer: inventory/preview only for debt on nightly; or force `apply=False` / disable content LLM digestion when invoked from `run_nightly`
- [ ] Do not reintroduce watch/`--with-llm` side doors
- [ ] Keep deterministic `compile_wiki` / `lint_wiki` on nightly
...
- [ ] Remove suggested_next_actions / batch hints that invoke deleted CLIs: `apply-action`, `review-action`, `apply-archive`, `apply-rewrite`, planner dry-run apply, etc.
- [ ] Keep live commands only: `file-back`, `alchemy-*`, single-page `review-page`, `compile`/`lint`/`ask` as appropriate
- [ ] Prefer not emitting `review-page --batch/--all-pending` as product hints (A21 residual)
...
- [ ] Sweep C53 / P9 / Evolution CLI tables that mark deleted commands as current
- [ ] Align USER_GUIDE review examples with thin 待审/已确认/废弃; file-back default judgment
- [ ] Note W7 in PROGRESS; checkboxes
