...
- 2026-08-05 (**收口第二波：F-11 巨函数单 seam 外提 + F-9 + 安全加固**)：concepts.py 1197→812（质量簇 7 函数 → `content/concept_quality.py` 402 行；`detect_concept_*_signals` 因 concepts→quality 环风险留在原处）；views.py 1178→942（判断资产簇 5 函数并入既有 `render/judgment_assets.py` →440）；`app_linting` phases.py 829→307（governance+curated 两 phase → `phases_governance.py` 537 行，core/`__init__` 直引 owner，无 re-export seam）；F-9：`_write_raw_response` 轮转（`_LLM_RAW_RESPONSE_KEEP=500`，UTC 文件名前缀字典序=时序，prune 失败仅告警不断写）+ 2 测试锁定；security：safe_fetch 301/302/303 对非 GET/HEAD 降级 GET 并丢 body/Content-*（307/308 保持）+ `PrivateAddressError(FetchPolicyError)` 结构化检测替代消息子串匹配 + 5 测试。`verify.sh all` 绿：acceptance 24 / llm 85 / Jest 203 / unit 72 / coverage 64%。
- 2026-08-04 (**全量扫描评估 6.8 + 4-agent 收口执行**)：报告 `docs/plans/2026-08-04-full-scan-evaluation.md`（综合 **6.8/10** vs 自评 9.05，实测覆盖率 58%）。修复落地——F-1：重建 main.js（W4 today-feed 修复此前未进产物，幻影 `proposal` kind 仍在分发）+ verify 加 bundle drift 硬门禁（正反向实测）+ `sync_product_shell_plugin` 前置 build；F-2：治理孤儿簇整簇删除（8 模块 ~4.6k 行：machine_memory_actions/batch/auto_resolution/concept_rewrite/archive/lifecycle/alchemy_migration/alchemy_cleanup + runner 4 wrapper + receipts/paths/alchemy_helpers 死符号；用户裁定删除）；F-3：`utils/security.py` 覆盖 39%→**99%**（56 例）；F-4：vault plugin/bootstrap 11 例 + verify 新增 `unit` 硬门禁（67 例）与 `coverage` 无门禁报告（实测 **64%**）；F-5：cli facade 43→7 行；F-6：classifier OS Independent→POSIX；F-7：verify 计数 79→82；F-8：`analyze_image` 补 429/5xx 重试 parity。read-only reviewer 复审（0 P0/P1）后收尾：DEVELOPER/Evolution 残留引用清剿、`autonomy_domains` 死模块删除、`analyze_image` 重试测试锁定（llm 82→83）、`verify_target_rules` 映射 unit、`docs_consistency_check` 新增计数钉（24/83/67/203）。`verify.sh all` 绿：acceptance 24 / llm 83 / Jest 203 / unit 67。Deferred：F-9 llm-responses 轮转、F-10~F-13（巨函数/文档归档/mirror 契约）。
