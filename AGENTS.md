...
- 常用 target：`scripts`、`smoke`、`python-static`、`acceptance`、`cli-smoke`、`product-shell-static`、`all`
  ...
  - `all` 走 `scripts + product-shell-static + cli-smoke + smoke + python-static + acceptance`（≈ 20–25 s，**含 acceptance 17 fixture replay + Product Shell Jest ~180**）；不含 pytest / coverage gate
