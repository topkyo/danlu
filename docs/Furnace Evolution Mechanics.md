...
- L3 proposal manual create / accept / revert（当前已有 manual baseline）；automatic generation / reject workflow（planned）
...
M3 当前收敛状态：manual baseline 已支持 `l3-proposal-create`、`review proposals`、`apply <proposal-id>` 与 `revert <receipt-id>`；写回范围限定为 `prompts/*.md` 与 `schema/policies/*`。`apply` 必须通过 proposal `before_hash`，失败时 proposal 转 `stale` 且不写目标；`revert` 必须通过 receipt `after_hash`，失败时转 `revert_conflict` 并生成 `human_merge_required` hint。自动 proposal generation、单独 reject workflow 和通用 audit stream 仍 deferred。
