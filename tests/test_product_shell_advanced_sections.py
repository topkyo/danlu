"""R91: Advanced 抽屉子 section 重组测试。

锁住三组结构（系统状态 / 运行与历史 / 开发者操作）+ dev banner 外置 +
...
class AdvancedSectionsStructureTests(unittest.TestCase):
    """render_advanced.js 三组结构 + dev banner 外置"""

    def test_render_advanced_drawer_uses_three_sections(self) -> None:
        text = (SRC / "render_advanced.js").read_text(encoding="utf-8")
        # 三个 key 都被 renderAdvancedSection 调用
        self.assertIn('key: "status"', text)
        self.assertIn('key: "history"', text)
        self.assertIn('key: "devops"', text)
        # 三组中文标题
        self.assertIn('plugin.t("系统状态")', text)
        self.assertIn('plugin.t("运行与历史")', text)
        self.assertIn('plugin.t("开发者操作")', text)
    ...
    def test_section_summary_helpers_exist(self) -> None:
        ...
        # 状态摘要 3 个核心字段
        self.assertIn("协议 {protocol} · LLM {llm} · 同步 {sync}", text)
        # 历史摘要含运行/审/执行计数
...
class AdvancedSectionsPersistenceTests(unittest.TestCase):
    ...
    def test_default_settings_includes_advanced_sections_expanded(self) -> None:
        ...
        # 默认全 false（默认折叠）
        self.assertIn("advancedSectionsExpanded: { status: false, history: false, devops: false }", text)
...
class AdvancedSectionsTranslationsTests(unittest.TestCase):
    ...
    def test_summary_template_keys_present(self) -> None:
        ...
        self.assertIn('"协议 {protocol} · LLM {llm} · 同步 {sync}"', text)
        self.assertIn('"最近运行 {n} 条 · 待审 {review} · 待执行 {execution}"', text)
        self.assertIn('"编译 / 同步 / 协议切换 / 日志等命令"', text)
...
class AdvancedSectionsNoOuterWrapperTests(unittest.TestCase):
    """R91 round 2 修复：去掉外层 Advanced details；DevOps 不再嵌旧 details"""
    ...
    def test_legacy_advanced_panel_is_no_longer_a_details(self) -> None:
        text = (SRC / "render_primitives.js").read_text(encoding="utf-8")
        idx = text.index("function renderLegacyAdvancedPanel(")
        end = text.index("\nfunction ", idx + 1)
        body = text[idx:end]
        # 不能再创建外层 details（避免 DevOps section 内嵌套折叠）
        self.assertNotIn('createEl("details"', body)
        # 但仍要保留核心按钮
        self.assertIn("Compile", body)
        self.assertIn("Nightly", body)
        self.assertIn("Set Protocol", body)
