...
def test_file_back_accepts_path_inside_vault(tmp_path: Path) -> None:
    ...
    report_path = vault / report_ref
    report_path.parent.mkdir(parents=True, exist_ok=True)
    report_path.write_text(f"{_FILE_BACK_DELIVERABLE_FM}# inside\n", encoding="utf-8")
...
def test_file_back_prefills_judgment_from_free_markdown(tmp_path: Path) -> None:
    ...
    report_path.write_text(
        f"{_FILE_BACK_DELIVERABLE_FM}# Growth scan\n\n{conclusion}\n",
        encoding="utf-8",
    )
...
def test_file_back_writes_slim_frontmatter_and_linked_supporting_artifact(tmp_path: Path) -> None:
    ...
    report_path.parent.mkdir(parents=True, exist_ok=True)
    report_path.write_text(
        f"{_FILE_BACK_DELIVERABLE_FM}# Slim FM\n\nJudgment from slim report.\n",
        encoding="utf-8",
    )
...
def test_review_page_confirm_lifts_judgment_from_linked_supporting_artifact(
    ...
    report_path.parent.mkdir(parents=True, exist_ok=True)
    report_path.write_text(
        f"{_FILE_BACK_DELIVERABLE_FM}# Linked confirm\n\nLinked judgment body for confirm.\n",
        encoding="utf-8",
    )
...
def test_review_page_confirm_rejects_placeholder_judgment(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    report_path = vault / report_ref
    report_path.parent.mkdir(parents=True, exist_ok=True)
    report_path.write_text(f"{_FILE_BACK_DELIVERABLE_FM}# Empty\n", encoding="utf-8")
...
def test_review_page_confirm_writes_review_history(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    report_path.parent.mkdir(parents=True, exist_ok=True)
    report_path.write_text(
        f"{_FILE_BACK_DELIVERABLE_FM}# Confirmable\n\nJudgment body ready for confirmation.\n",
        encoding="utf-8",
    )
...
def test_file_back_judgment_preserves_derived_promoted_to(  # pragma: no cover - explicit pytest acceptance gate
    ...
    report_path.parent.mkdir(parents=True, exist_ok=True)
    report_path.write_text(
        f"{_FILE_BACK_DELIVERABLE_FM}# D3 old report\n\nSeed report for file-back regression.\n",
        encoding="utf-8",
    )
