...
`.codex/review-capabilities.env` is the local capability map for `resolve_review_mode.sh`; by default it keeps review conservative and resolves to `same-context` until you explicitly mark a more isolated reviewer mode as available. Review 默认应优先使用 Claude Opus 4.6；当 Claude 路径不可用时，Codex fresh-session preset 作为 GPT-5.4 fallback。The same file can also define `REVIEW_LAUNCH_COMMAND_*` hooks for `launch_qa_review.sh`; the bundled template already ships an opt-in `fresh-session` preset using `codex review -c model="gpt-5.4" --uncommitted`, and it writes review output under `.codex/review-handoffs/qa-review.output.md`. `run_qa_review.sh` is the convenience wrapper that consumes the same preset, writes the final gate artifact, and can append calibration in the same command.
