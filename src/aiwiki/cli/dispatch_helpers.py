...
def _classify_review_bucket(entry: FeedEntry) -> str:
    ...
    Sub-bucket 来源：
    - target 形如 "review:<x>" → "<x>" (e.g. concept_backlog, revisit, machine_memory_actions, judgment_review)
...
def _action_command(item: dict[str, object]) -> str:
    _ = item
    return "PYTHONPATH=src python3 -m aiwiki.cli --root . advanced review-queue --bucket machine_memory_actions --json"
...
def _action_review_item(item: dict[str, object]) -> dict[str, object]:
    ...
    target = str(item.get("primary_path") or item.get("secondary_path") or action_id)
    return {
        "id": action_id,
...
def _review_queue_detail_buckets(
    ...
    if isinstance(execution_controls, dict):
        ...
        if has_backlog("machine_memory_actions"):
            buckets["machine_memory_actions"] = [_action_review_item(item) for item in actions]
