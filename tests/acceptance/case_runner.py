...
# Frozen acceptance goldens assume no usable LLM backend: `LLMConfig.from_env()`
# resolves the default backend and finds no key (see `expected/files/*.golden`).
# Host shells exporting backend selections or any provider credential change
# resolution, probe behavior, and hint strings — strip those keys only for
# in-process CLI runs. (The default backend is `deepseek-api`; a dev machine
# exporting DEEPSEEK_API_KEY would otherwise light up real LLM calls.)
