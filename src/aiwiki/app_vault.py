    ...
    return (
        "\n".join(
            [
                ...
                "- Product Shell 默认 LLM route 是 `opencode-api/deepseek-v4-pro`；key 默认走 `AIWIKI_OPENCODE_API_KEY`，也可用通用 `AIWIKI_LLM_API_KEY`。",
                "- NVIDIA NIM 保留为 unattended fallback route：`nvidia-nim-api/openai/gpt-oss-120b`。",
                "- 常用 API provider：OpenCode / NVIDIA NIM / OpenRouter / Anthropic；高级项：Codex CLI / Copilot CLI / Claude CLI / 自定义 OpenAI-compatible。",
...
def _render_launcher_script(runtime_root: Path) -> str:
    ...
    return "\n".join(
        [
            ...
            "profiles = {",
            '    "opencode-api": {"model": "deepseek-v4-pro", "key_setting": "llmOpencodeApiKey", "key_env": "AIWIKI_OPENCODE_API_KEY", "base_setting": "llmOpencodeBaseUrl", "base_env": "AIWIKI_OPENCODE_BASE_URL"},',
            '    "nvidia-nim-api": {"model": "openai/gpt-oss-120b", "key_setting": "llmNvidiaNimApiKey", "key_env": "AIWIKI_NVIDIA_NIM_API_KEY", "base_setting": "llmNvidiaNimBaseUrl", "base_env": "AIWIKI_NVIDIA_NIM_BASE_URL"},',
            '    "openrouter-api": {"model": "", "key_setting": "llmOpenrouterApiKey", "key_env": "AIWIKI_OPENROUTER_API_KEY", "base_setting": "llmOpenrouterBaseUrl", "base_env": "AIWIKI_OPENROUTER_BASE_URL"},',
            '    "anthropic-api": {"model": "claude-sonnet-4-20250514", "key_setting": "llmAnthropicApiKey", "key_env": "AIWIKI_ANTHROPIC_API_KEY", "base_setting": "llmAnthropicBaseUrl", "base_env": "AIWIKI_ANTHROPIC_BASE_URL"},',
            '    "openai-api": {"model": "gpt-4.1-mini", "key_setting": "llmCustomOpenaiApiKey", "key_env": "AIWIKI_LLM_API_KEY", "base_setting": "llmCustomOpenaiBaseUrl", "base_env": "AIWIKI_LLM_BASE_URL"},',
            "}",
            ...
            'mapping = {"AIWIKI_LLM_BACKEND": backend, "AIWIKI_LLM_MODEL": configured_model or profile_model}',
