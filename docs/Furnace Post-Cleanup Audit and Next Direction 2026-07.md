...
| Tests | acceptance **25** + llm-integration **84** + unit **177** + Jest **204**（2026-08-07 入口面清理 follow-up 实测；历史快照曾含 24/85/166、24/85/160、24/85/154、24/85/143 等） | `bash scripts/verify.sh all` |
...
| D5 | Product Shell Jest 默认可 soft-skip | `scripts/run_product_shell_tests.sh`（本轮清理删除） | `verify` 可绿但 UI 回归未跑 | **fixed 2026-07-15**：Jest hard-gate（当时 168；现行见 §1：**204**） |
...
| D14 | JS 行为测试偏弱（grep token） | Round 92.8 Residual | plugin 大改回归弱 | **improved**：Jest hard-gate（当时 168；现行 **204**）；行为覆盖仍可加深 |
...
# Product Shell：`product-shell-static` = node --check + bundle drift 硬门禁 + Jest hard-gate（204 tests；见 `bash scripts/verify.sh product-shell-static`）
# 紧急旁路：AIWIKI_SKIP_PRODUCT_SHELL_JS_TESTS=1
# go-live 文档门禁：
! git grep -E 'commercial@example\.com|support@example\.com' -- LICENSE docs/commercial/
bash scripts/verify.sh all   # 推送/发布前（含 unit 177 + coverage informational）
```
...
- [x] Jest 在 release CI 为 hard-gate（`verify.sh product-shell-static`；**204** passed + bundle drift gate）
...
- 2026-08-05：优先债收口（不含 Commercial 三阻断）— §1 对齐 **187** py / **~43.8k** / unit **153** / 工程实测 **8.2**；coverage 快照 **69%**；`docs_consistency` 钉 Post-Cleanup unit 153。
