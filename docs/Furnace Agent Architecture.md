...
| planner | partial | 当前已有 `.aiwiki/state/planner-state.json` 的 repair/execution proposal planner，以及 `.aiwiki/state/planner-log.jsonl` 的 signal observe-only decision log；`planner-log-rollback --dry-run` 可只读预览 future append marker，但不删除或重写日志；完整 scheduler execute mode 未落地。 |
