...
def load_planner_state(root: Path) -> dict[str, Any]:
    ...
    if not isinstance(dependency_graph, dict) or not isinstance(counts, dict) or not isinstance(executed_actions, list):
        return default_planner_state()
    # Live apply queues are retired. Keep schema + executed_actions history, but
    # never echo frozen vault leftovers as current planner work.
    executed = [item for item in executed_actions if isinstance(item, dict)]
    return {
