...
from .run_notes import run_id_for_artifact, write_run_notes_frontmatter


def _reuse_unsuccessful_ask_report(path: Path) -> bool:
    try:
        frontmatter = parse_frontmatter(path.read_text(encoding="utf-8", errors="replace"))
    except OSError:
        return False
    quality = str(frontmatter.get("artifact_quality") or "").strip()
    status = str(frontmatter.get("llm_status") or "").strip()
    return quality in {"no-evidence", "placeholder", "degraded"} or status in {
        "pending",
        "failed",
        "degraded",
...
def _materialize_ask_report_artifact(
    ...
    *,
    ...
    artifact_id = next_available_stem(directory, artifact_seed, reuse_existing=_reuse_unsuccessful_ask_report)
...
def _finalize_ask_question(
    ...
    *,
    ...
    write_run_notes_frontmatter(destination, run_id=run_id)
