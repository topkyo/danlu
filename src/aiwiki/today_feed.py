...
def build_today_feed(summary: dict[str, Any], *, audience: FeedAudience = "primary") -> list[FeedEntry]:
    ...
    entries.extend(_build_coverage_gap_entries(summary, today_date))
    entries.extend(_build_watcher_stale_entries(summary))
...
def _is_deliverable_report_output(item: dict[str, Any]) -> bool:
    ...
    if background_status == "degraded":
        return False
    if artifact_quality in {"degraded", "placeholder", "no-evidence"}:
        return False
...
def _build_coverage_gap_entries(summary: dict[str, Any], today_date: str) -> list[FeedEntry]:
    entries: list[FeedEntry] = []
    for item in _dict_items(summary.get("recent_outputs")):
        if _first_text(item, "artifact_quality") != "no-evidence":
            continue
        timestamp = _first_text(item, "generated_at", "created_at")
        if _date_part(timestamp) != today_date:
            continue
...
def _build_watcher_stale_entries(summary: dict[str, Any]) -> list[FeedEntry]:
    watcher = summary.get("watcher")
    if not isinstance(watcher, dict) or not watcher.get("stale"):
        return []
    return [
        FeedEntry(
            kind="action",
            ...
            target=str(watcher.get("state_path") or ".aiwiki/state/automation.json"),
            timestamp=str(summary.get("generated_at") or ""),
            protocol="",
        )
    ]
