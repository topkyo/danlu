...
实现契约 SoT：active corpus、金丹生命周期、现行 operator CLI。**不**复述已删 AgentOS 命令表；heavy/light lane、signals/planner-log ops、L3 apply/revert CLI 见 [[docs/Furnace Agent Architecture|架构文档]] §8。
...
**不定义**：Product Shell UI 细节、终局愿景（见架构文档）。
...
> 金丹：只走 `advanced alchemy start|distill|finalize|promote|revert|demote`（见 `docs/USER_GUIDE.md`）。
...
| 状态 | 路径 | 入口 |
|---|---|---|
| `draft` | `.aiwiki/staging/elixirs/` | `advanced alchemy start <corpus_id> --topic …` |
| `distilling` | 同上 | `advanced alchemy distill <elixir_id> --question …` |
| `candidate` | 同上 | `advanced alchemy finalize <elixir-id>` |
| `settled` | `wiki/elixirs/` | `advanced alchemy promote --elixir-id …` |
...
- **可 revert**：金丹 promotion（`advanced alchemy revert`）；历史 L3 apply 链已删 CLI。
...
- Signal / planner / heavy-light 章节描述**目标契约**；已落地 subset 与 W3 裁剪以 `docs/DEVELOPER.md` 为准。
